...
def evaluate_openwebtext_validation_loss(
    ...
    def evaluate_edge_losses_fn(
        *,
        ...
        def compute_translated_loss_value() -> float:
            translated_past = translator_pool.translate_top_layers(
                past_key_values=past_by_node_id[edge.src_id],
                src_node_id=edge.src_id,
                tgt_node_id=edge.tgt_id,
                tgt_spec=ctx.mm.get_model_spec(edge.tgt_id),
            ...
            mixed_target_past = replace_top_layers(
                base_past_key_values=past_by_node_id[edge.tgt_id],
                translated_top_past_key_values=translated_past,
            )
            return float(
    ...
    def build_visualization_pasts_fn(
        ...
        return build_openwebtext_tsne_named_pasts(
            source_top_past_key_values=source_top_past,
            translated_past_key_values=translated_past,
            target_top_past_key_values=target_top_past,
        )
