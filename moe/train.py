...
class DenseMoEFeedForward(nn.Module):
    """
    Token-level MoE feed-forward network used inside each translator block.

    The router gates dense MLP experts, not translators. `num_experts` and `top_k`
    are currently wired to the existing MOT config fields (`mot_num_translators`
    and `mot_top_k`) to avoid changing config files while the experiment is in
    flight.
    """

    def __init__(self, dim: int, mlp_ratio: int, num_experts: int, top_k: int) -> None:
        super().__init__()
        if num_experts < 1:
        ...
        router_hidden_dim = max(64, min(dim, hidden_dim))
    ...
    def _compute_mixture_weights(self, hidden: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, hidden: torch.Tensor) -> torch.Tensor:
        if hidden.ndim != 3:
            raise ValueError(f"DenseMoEFeedForward expects [batch, seq, dim], got {tuple(hidden.shape)}")
        mixture_weights = self._compute_mixture_weights(hidden)
        expert_outputs = torch.stack([expert(hidden) for expert in self.experts], dim=2)
        return (expert_outputs * mixture_weights.unsqueeze(-1)).sum(dim=2)
    ...
    def get_balance_metrics(self) -> Optional[Dict[str, float]]:
        ...
        normalized_importance = token_importance / token_importance.sum().clamp_min(eps)
        return {
            "gate_importance_cv2": float(squared_cv(token_importance).item()),
            "gate_load_cv2": float(squared_cv(token_load).item()),
            "gate_importance_entropy": float((-(normalized_importance) * normalized_importance.clamp_min(eps).log()).sum().item()),
        }


class MoECrossAttentionBlock(CrossAttentionBlock):
    """Cross-attention block whose dense FFN is replaced by a token-level MoE FFN."""

    def __init__(self, dim: int, num_heads: int, mlp_ratio: int, num_experts: int, top_k: int) -> None:
        super().__init__(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio)
        self.ffn = DenseMoEFeedForward(
            dim=dim,
            mlp_ratio=mlp_ratio,
...
class CrossLayerWindowTranslator(nn.Module):
    ...
    For MOT, the translator remains a single module and only the internal dense
    FFN in each cross-attention block is replaced by MoE experts.
    """
    ...
    def __init__(
        ...
        if ffn_num_experts < 1:
            raise ValueError("ffn_num_experts must be >= 1")
        if ffn_top_k < 1:
            raise ValueError("ffn_top_k must be >= 1")
        if ffn_top_k > ffn_num_experts:
            raise ValueError("ffn_top_k must be <= ffn_num_experts")
    ...
    def _build_block(self, *, dim: int, num_heads: int, mlp_ratio: int) -> nn.Module:
        if self.ffn_num_experts == 1:
            return CrossAttentionBlock(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio)
        return MoECrossAttentionBlock(
            dim=dim,
            num_heads=num_heads,
            mlp_ratio=mlp_ratio,
            num_experts=self.ffn_num_experts,
            top_k=self.ffn_top_k,
        )
...
def collect_mot_balance_metrics(module: nn.Module) -> Dict[str, float]:
    ...
    for submodule in module.modules():
        if not isinstance(submodule, DenseMoEFeedForward):
            continue
    ...
    if num_moe_modules == 0:
        return {}
    return {name: value / num_moe_modules for name, value in summed_metrics.items()}
    ...
    *,
    ...
    if variant == "mot":
        return translator_cls(
            ...
            translator_depth=translator_depth,
            mlp_ratio=mlp_ratio,
            ffn_num_experts=mot_num_translators,
            ffn_top_k=mot_top_k,
        )
