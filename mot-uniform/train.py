...
class MixtureOfTranslators(nn.Module):
    def __init__(
        ...
        # Uniform-gated MOT keeps the translator pool structure but removes the
        # learned router. Every translator receives the same weight for every
        # token, so mot_top_k is accepted for config/checkpoint compatibility
        # but does not change the active translator set in this variant.
    ...
    def _compute_mixture_weights(self, layer_window_cache: torch.Tensor) -> torch.Tensor:
        ...
        mixture_weights = torch.full(
            (batch_size, seq_len, self.num_translators),
            fill_value=1.0 / self.num_translators,
            dtype=layer_window_cache.dtype,
            device=layer_window_cache.device,
        )
        self.last_router_logits = torch.zeros_like(mixture_weights).detach()
